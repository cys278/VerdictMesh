...
from verdictmesh import Agent, Aggregator, Engine
import datetime
...
class Cardiologist(Agent):
    def __init__(self, model=MODEL):
        ...
        self.llm = lambda prompt: generate(prompt, model=model)
    def execute(self, medical_report: str) -> str:
        return self.llm(prompts.CARDIOLOGIST_PROMPT.format(medical_report=medical_report))
...
class Psychologist(Agent):
    def __init__(self, model=MODEL):
        ...
        self.llm = lambda prompt: generate(prompt, model=model)
    def execute(self, medical_report: str) -> str:
        return self.llm(prompts.PSYCHOLOGIST_PROMPT.format(medical_report=medical_report))
...
class Pulmonologist(Agent):
    def __init__(self, model=MODEL):
        ...
        self.llm = lambda prompt: generate(prompt, model=model)
    def execute(self, medical_report: str) -> str:
        return self.llm(prompts.PULMONOLOGIST_PROMPT.format(medical_report=medical_report))
...
class Neurologist(Agent):
    def __init__(self, model=MODEL):
        ...
        self.llm = lambda prompt: generate(prompt, model=model)
    def execute(self, medical_report: str) -> str:
        return self.llm(prompts.NEUROLOGIST_PROMPT.format(medical_report=medical_report))
...
async def download_log(session_id: str):
    ...
    log_content = (
        "=================================================\n"
        " VERDICTMESH AI - EU AI ACT COMPLIANCE TRACE LOG \n"
        "=================================================\n"
    ...
    headers = {
        "Content-Disposition": f"attachment; filename=verdictmesh_trace_{session_id}.txt"
    }
