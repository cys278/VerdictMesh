...
from verdictmesh.nvidia import get_client, MODEL
...
def main():
    ...
    print("🚨 Global Supply Chain Crisis Detected. Initiating VerdictMesh...")
    ...
    def call_nvidia_simple(prompt: str) -> str:
        res = client.chat.completions.create(
            model=MODEL,
            messages=[{"role": "user", "content": prompt}],
            temperature=0.1, reasoning_effort="low", max_tokens=2048
        )
        return res.choices[0].message.content

    boss = Synthesizer(
        llm_callable=call_nvidia_simple,
